...
namespace GoFishTests
{
	...
	public class PlayerTests	
	{
		...
		public void TestAddCardsAndPullOutBooks()
		{
			...
			CollectionAssert.AreEqual(new List<Values>() 
			{Values.Three, Values.Jack     }, books);
		...
		[TestMethod]
		public void TestDrawEmptyCard()
		{
			//test against empty deck
			var player = new Player("Owen", new List<Card>());
		...
		public void TestRandomValueFromHand()
		{
			...
			Player._=new MockRandom() { ValueToReturn=0 };
			Assert.AreEqual("Ace", player.RandomValueFromHand().ToString());
			Player._ = new MockRandom() { ValueToReturn=4 };
			Assert.AreEqual("Two", player.RandomValueFromHand().ToString());
			Player._=new MockRandom() { ValueToReturn=8 };
